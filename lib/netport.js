	...
	var v = {
		...
		'&' : function() {
			...
			if ( m[cur$c].length==1 ) {
				...
				cur$c++;
				return function() {
					t.p.netpost$n.send$s(this.__ID__, n, arguments);
				}
			} else {
				var n = m[cur$c++].substr(1);
				return function() {
					t.p.netpost$n.send2$s2(n, arguments);
				}
			}
		},
		// value
...
NetPort$N.prototype.serialize$z = function(args$a) {
	...
	if (!(o instanceof Object)) {
		r += VALSEP$VS + t.encode$e(o);
	} else {
		r += PRESEP$PS + t.p.store$s.regist$r(o);
		if (o instanceof Array) {
			for ( var i=0; i<o.length; i++ ) {
				r += arguments.calee(o[i]);
			}
		} else if (!(o instanceof Function)) {
			for ( var i in o ) {
				r += NAMESEP$VS + t.encode$e(i) + arguments.calee(o[i]);
			}
		}
		r += SUFSEP$PS;
	}
...
//	}(cd$c + NAMESEP$NS + t.encode$e(name$n), args$a);
	...
	this.p.sock$w.send(PRESEP$PS + cd$c + NAMESEP$NS + t.encode$e(name$n) + this.serialize$z(args$a));
}
...
NetPort$N.prototype.create$c = function(name$n, func$f) {
	this.p.sock$w.send( ITCSEP$IS + name$s + ITCSEP$IS + 
