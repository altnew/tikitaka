...
NetPort$N.prototype.receive$r = function(m) {
	...
	var s = function(n, nn) {
		...
		var f = {
			...
			1 : function() {
				__TLOG('sf1',p);
				var l, n, r ={__ID__ : m.subarray(++p, p+4).toN()};
				p += 4;
				while (p<m.length) {
					l = m[p++];
			...
			3 : function(n) {
				__TLOG('sf3',p);
				var id = m.subarray(++p, p+4).toN();
				p += 4;
				return (id==0) ?
					function() { t.p.netport$n.sendO$s1(C.frN(this.__ID__,4), n, arguments) } :
					function() { t.p.netport$n.sendF$s2(C.frN(id,4), arguments) };
			},
			// string
			4 : function() {
				__TLOG('sf4',p);
...
NetPort$N.prototype.sendA$s0 = function(d,a) {
	var t = this;
	d.push(0x02);
	d.push({l:4,d:C.frN(a.length,4)});
	var f = function(m, p) {
		if (m instanceof Uint8Array) {
			d.push(0x05);
			d.push({l:4, d:C.frN(m.length,4)});
			d.push(m);
		} else if (m instanceof Function) {
			var id = p ? 0 : t.p.store$s.regist$r(m);
			d.push(0x03);
			d.push({l:4, d:C.frN(id,4)});
		} else if (typeof m === typeof '') {
			var mm = C.frS(m);
			d.push(0x04);
			d.push({l:4,d:C.frN(mm.length,4)});
			d.push(mm);
			...
			for (var i in m) {
				var ii = C.frS(i);
				d.push({l:1,d:C.frN(ii.length,1)});
				d.push(ii);
				f(m[i], true);
			}
		} else {
			var n = C.frS(m[i].toString());
			d.push(0x04);
			d.push({l:4,d:C.frN(n.length,4)});
			d.push(n);
		}
	};
...
NetPort$N.prototype.sendO$s1 = function(id, n, a) {
	var nn = C.frS(n);
	__TLOG('sendO$s1', nn);
	this.p.netport$n.sendA$s0([0x02, id, {l:1,d:C.frN(nn.length,1)}, nn],a);
}
...
NetPort$N.prototype.sendC$s3 = function(n, f) {
	...
	this.p.netport$n.send$s([0x01, {l:1, d:C.frN(nn.length,1)}, nn, {l:4, d:C.frN(id,4)}]);
}
